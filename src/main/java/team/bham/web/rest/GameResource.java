...
import team.bham.service.GameService;
...
/**
 * REST controller for managing {@link team.bham.domain.Game}.
 */
@RestController
@RequestMapping("/api")
public class GameResource {
    ...
    /**
     * {@code GET  /games//match/:id} : get the games from the "id" match.
     *
     * @param id the matchId of the gameDTOs to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of games in the body.
     */
    @GetMapping("/games/match/{id}")
    public List<GameDTO> getGameByMatchId(@PathVariable Long id) {
        log.debug("REST request to get Games with MatchId : {}", id);
        return gameService.findAllGamesByMatchId(id);
    }
