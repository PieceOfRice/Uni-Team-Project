...
import team.bham.service.GamePlayerService;
...
/**
 * REST controller for managing {@link team.bham.domain.GamePlayer}.
 */
@RestController
@RequestMapping("/api")
public class GamePlayerResource {
    ...
    public ResponseEntity<GamePlayerDTO> updateGamePlayer(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, gamePlayerDTO.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<GamePlayerDTO> partialUpdateGamePlayer(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, gamePlayerDTO.getId().toString())
        );
    }
    ...
    /**
     * {@code GET  /game-players} : get all the gamePlayers in "id" game.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of gamePlayers in body.
     */
    @GetMapping("/game-players/game/{id}")
    public List<GamePlayerDTO> getGamePlayerByGameId(@PathVariable Long id) {
        log.debug("REST request to get all GamePlayers From Game id : {}", id);
        return gamePlayerService.findAllGamePlayerByGameId(id);
    }
    ...
    /**
     * {@code DELETE  /game-players/game/:id} : delete the "id" gamePlayer.
     *
     * @param id the id of the gamePlayerDTO to delete.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/game-players/game/{id}")
    public ResponseEntity<Void> deleteByGameId(@PathVariable Long id) {
        log.debug("REST request to delete GamePlayer by GameID : {}", id);
