...
import { EntityArrayResponseType, TeamPlayerService } from '../service/team-player.service';
...
export class TeamPlayerComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITeamPlayer): number => this.teamPlayerService.getTeamPlayerIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.teamPlayers = this.refineData(dataFromBody);
  }

  protected refineData(data: ITeamPlayer[]): ITeamPlayer[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ITeamPlayer[] | null): ITeamPlayer[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.teamPlayerService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
